...
def simulated_annealing(number):
    ...
    def simulated_annealing(temperature, alpha, max_iterations):
        ...
        for iteration in range(max_iterations):
            ...
            # If the neighbour is better it is always accepted, else it is accepted with probability e^(-delta/T)
            if delta <= 0 or random.random() < math.exp(-delta / temperature):
                current_list = neighbour_list
                current_cost = neighbour_cost

            # If the current solution is better than the best solution it is updated
    ...
    ticks = list(range(0, max_time + 10, 10))
    ax.set_xticks(ticks)
    labels = [str(x) if x % 50 == 0 else "" for x in ticks]
    ...
    image_name = f"gantt_{number}.png"
