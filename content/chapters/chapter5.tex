...
In \textbf{Figure \ref{fig:instance_45}} is shown one of the most complex instances (19 jobs and 10 machines). No solution was found in the MIP solver in the timer set and the reason is clearly visible from the density and complexity of the solution given by the metaheuristic.
...
\newpage
In \textbf{Figures \ref{fig:instance_40} and \ref{fig:instance_40mip}} are shown the charts of an instance simple enough to let the MIP solver find a solution. The makespan of the metaheuristic and of the exact solver is the same but the positions of the jobs are not. This means that there isn't only one optimal solution or a single optimal job order. Here in particular four of the seven jobs have different order but the makespan is in both solutions optimal.
